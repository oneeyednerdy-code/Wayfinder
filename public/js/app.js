...
function showAuthResult() {
  ...
  if (auth === 'connected') toast('Twitch connected securely.', 'success');
    ...
    const messages = {
      ...
      invalid_client: 'Twitch rejected the application credentials. Check TWITCH_CLIENT_ID and TWITCH_CLIENT_SECRET.',
      invalid_scope: 'Twitch rejected the requested scope. Wayfinder should request only openid.',
      invalid_request: 'Twitch rejected the authorization request. Check the configured callback URL and application settings.',
      temporarily_unavailable: 'Twitch authorization is temporarily unavailable. Try again shortly.',
      server_error: 'Twitch returned an authorization server error. Try again shortly.',
      oauth_error: 'Twitch authorization failed before Wayfinder could establish a session.',
    };
...
async function init() {
  ...
  document.querySelector('#twitch-connect').addEventListener('click', async (event) => {
    // When disconnected, this is a real link to /api/auth/login. Do not depend on JS to start OAuth.
